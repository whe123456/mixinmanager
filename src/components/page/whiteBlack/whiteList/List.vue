<template>
    <div>
        <el-form :inline="true" class="demo-form-inline">
            <el-form-item>
                <el-button type="primary" @click="add">添加白名单</el-button>
...
<script>
	import {deleteWhiteUser, whiteList} from '@/api/api'
	...
	export default {
		...
		methods: {
			add() {
				this.$router.push('/whiteBlack/whiteAdd')
			},
			...
			del(e, type = 1) {
				...
				this.$confirm(msg, '提示', {
					...
					that.loading = true
					deleteWhiteUser({phone: uid}).then(response => {
						that.loading = false
